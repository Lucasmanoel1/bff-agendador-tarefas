...
public class CronService {
    ...
    public void buscaTarefasProximaHora(){
        ...
        List<TarefasDTOResponse> listaTarefas = tarefasService.buscaListaTarefasAgendadasPorPeriodo(horaAtual, horaFutura, token);
        log.info("Tarefas encontradas " + listaTarefas);

        listaTarefas.forEach(tarefa -> {
                                emailService.enviarEmail(tarefa);
                                log.info("Email enviado para o usuario" + tarefa.getEmail());
                                tarefasService.alteraStatusTarefa(StatusNotificacaoEnum.NOTIFICADO,
                                                                    tarefa.getId(), token);});
